...
server.get('*', (req, res, next) => {
  ...
  match({ routes, location }, (error, redirectLocation, renderProps) => {
    function getReduxPromise () {
      ...
      let comp
      let promise
      comp = renderProps.components[renderProps.components.length - 1].WrappedComponent
      promise = comp.fetchData ? comp.fetchData({ query, params, store, history }) : Promise.resolve()
    ...
    if (redirectLocation) {
      ...
      getReduxPromise().then(() => {
        ...
        let html = ReactDOMServer.renderToString(<Provider store={store}>{ <RouterContext {...renderProps} /> }</Provider>)
